...
from utils.network_wrappers import (
    Detector,
    VectorCNN,
)
...
def check_detectors(args):
    ...
    non_empty_detectors = [
        (det, value) for det, value in detectors.items() if value
    ]
    det_number = len(non_empty_detectors)
    if det_number == 0:
        log.error(
            "No detector specified, please specify one of the following parameters: "  # noqa: E501
        ...
        log.error(
            "Only one detector expected but got {}, please specify one of them:{}".format(  # noqa: E501
...
def save_json_file(save_path, data, description=""):
    ...
    if description:
        log.debug(f"{description} saved to {save_path}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Multi camera multi object \
    ...
    parser.add_argument(
        ...
        required=True,
        nargs="+",
        help="Required. Input sources (indexes of cameras or paths to video files)",  # noqa: E501
    )
    parser.add_argument(
        "--loop",
        default=False,
    ...
    parser.add_argument(
        ...
        default="",
        required=False,
        help="Optional. Path to file in JSON format to save results of the demo",  # noqa: E501
    )
    parser.add_argument(
        "--save_detections",
        type=str,
    ...
    if len(args.config):
        ...
        log.error(
            "Please specify parameter '--config'",
        )
        sys.exit(1)
