...
class MultiCameraTracker:
    def __init__(
        ...
        self.time_window = (
            time_window  # should be greater than time window in scts
        )
        ...
        while len(indices_rows) > 0 and len(indices_cols) > 0:
            ...
            if dist < self.global_match_thresh:
                ...
                if all_tracks[idx1].id > all_tracks[idx2].id:
                    self.scts[all_tracks[idx1].cam_id].check_and_merge(
                        all_tracks[idx2],
                        all_tracks[idx1],
                    )
                else:
                    self.scts[all_tracks[idx2].cam_id].check_and_merge(
                        all_tracks[idx1],
                        all_tracks[idx2],
                    )
                assert i != j
    ...
    def _compute_mct_distance_matrix(self, all_tracks):
        ...
        for i, track1 in enumerate(all_tracks):
            for j, track2 in enumerate(all_tracks):
                ...
                if (
                    track1.id != track2.id
                    and track1.cam_id != track2.cam_id
                    and len(track1) > self.time_window
                    and len(track2) > self.time_window
                    ...
                    if not track1.f_orient.is_valid():
                        ...
                        f_complex_dist = track1.f_orient.dist_to_other(
                            track2.f_orient,
                        )
                    f_avg_dist = 0.5 * cosine(
                        track1.f_avg.get(),
                        track2.f_avg.get(),
                    )
