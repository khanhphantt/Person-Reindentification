...
class MaskRCNN(DetectorInterface):
    ...
    def __init__(
        ...
        self.n, self.c, self.h, self.w = self.model.input(
    ...
    def preprocess(self, frame):
        ...
        return namespace(
            ...
            im_info=np.array(
                [processed_image.shape[1], processed_image.shape[2], 1.0],
                dtype="float32",
            ),
        )
    ...
    def forward(self, im_data, im_info):
        if (self.h - im_data.shape[1] < 0) or (self.w - im_data.shape[2] < 0):
            raise ValueError(
                "Input image should resolution of {}x{} or less, "
                "got {}x{}.".format(
                    self.w,
                    self.h,
                    im_data.shape[2],
        ...
        im_data = np.pad(
            ...
            mode="constant",
            constant_values=0,
        ).reshape(1, self.c, self.h, self.w)
        ...
        if self.segmentoly_type:
            output = {
                ...
                for name in self.output_keys_segmentoly
            }
        ...
        return (
            ...
            classes,
            scores,
            np.full(len(classes), 0, dtype=np.int32),
            masks,
        )
        ...
        for source in self.detections:
            ...
            if self.last_index in source:
                for bbox, score in zip(
                    source[self.last_index]["boxes"],
                    source[self.last_index]["scores"],
                ):
