...
class Analyzer:
    def __init__(
        ...
        self.save_distances = (
            os.path.join(save_distances, f"sct_{cam_id}")
            if len(save_distances)
            else ""
        )
        self.save_timeline = (
            os.path.join(save_timeline, f"sct_{cam_id}")
            if len(save_timeline)
            else ""
        )
    ...
    def prepare_distances(self, tracks, current_detections):
        ...
        for j, dist_img in enumerate(self.distance_imgs):
            ...
            for n, i in enumerate(
                range(0, dist_img.shape[1] + 1, self.crop_size[0]),
            ):
    ...
    def show_all_dist_imgs(self, time, active_tracks):
        ...
        if self.concatenate_distances:
            ...
            concatenated_dist_img = np.hstack(
                [self.distance_imgs[i] for i in range(0, 3)],
            )
            concatenated_iou_am_img = np.hstack(
                [self.distance_imgs[i] for i in range(3, 5)],
            )
            empty_img = np.full(
                self.distance_imgs[2].shape,
                225,
                dtype="uint8",
            )
            concatenated_iou_am_img = np.hstack(
                [concatenated_iou_am_img, empty_img],
            )
            concatenated_dist_img = np.vstack(
                [concatenated_dist_img, concatenated_iou_am_img],
            )

        if self.show_distances:
            if concatenated_dist_img is not None:
                cv2.imshow(
                    f"SCT_{self.id}_Distances",
                    concatenated_dist_img,
                )
            else:
                for i, img in enumerate(self.distance_imgs):
        ...
        if len(self.save_distances):
            if concatenated_dist_img is not None:
                ...
                for i, img in enumerate(self.distance_imgs):
                    file_path = os.path.join(
                        self.save_distances,
                        f"frame_{time}_{self.dist_names[i]}.jpg",
                    )
...
def save_embeddings(scts, save_path, use_images=False, step=0):
    def make_label_img(label_img, crop, target_size=(32, 32)):
        ...
        label_img = (
            ...
            else np.concatenate((label_img, np.expand_dims(img, 0)))
        )
        return label_img
    ...
    for i, sct in enumerate(scts):
        for track in tqdm(
            sct.tracks,
            f"Processing embeddings: SCT#{i}...",
        ):
            if use_images and len(track.crops) == 1 and track.crops[0] is None:
                log.warning(
                    "For embeddings was enabled parameter 'use_images' but images were not found!"  # noqa: E501
                    "'use_images' switched off. Please check if parameter 'enable' for analyzer"  # noqa: E501
                    "is set to True",
                )
                use_images = False
            ...
            if isinstance(track.f_avg.avg, int):
                continue
            embeddings_avg = (
                track.f_avg.avg.reshape((1, -1))
                if embeddings_avg is None
                else np.concatenate(
                    (embeddings_avg, track.f_avg.avg.reshape((1, -1))),
                )
            )
            metadata_avg.append(f"sct_{i}_" + str(track.id))
            if use_images:
            ...
            for j, f in enumerate(track.features):
                ...
                features = (
                    f.reshape((1, -1))
                    if features is None
                    else np.concatenate((features, f.reshape((1, -1))))
                )
            ...
            embeddings_all = (
                ...
                else np.concatenate((embeddings_all, features))
            )
            # Collect clustered embeddings
            for j, f_clust in enumerate(track.f_clust.clusters):
                embeddings_clust = (
                    f_clust.reshape((1, -1))
                    if embeddings_clust is None
                    else np.concatenate(
                        (embeddings_clust, f_clust.reshape((1, -1))),
                    )
                )
                metadata_clust.append(str(track.id))
                if use_images:
                    label_img_clust = make_label_img(
                        label_img_clust,
                        track.crops[j],
                    )
