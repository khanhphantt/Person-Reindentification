...
def postprocess(
    ...
    if raw_cls_masks is not None:
        raw_cls_masks = [
            segm
            for segm, is_valid in zip(raw_cls_masks, confidence_filter)
            if is_valid
        ]
...
def segm_postprocess(
    box,
    raw_cls_mask,
    im_h,
    im_w,
    full_image_mask=True,
    encode=False,
):
    ...
    if full_image_mask:
        ...
        im_mask = np.ascontiguousarray(
            mask[
                (y0 - original_box[1]) : (y1 - original_box[1]),
