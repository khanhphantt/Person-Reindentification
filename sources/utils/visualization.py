...
def draw_detections(frame, detections, show_all_detections=True):
    ...
    for i, obj in enumerate(detections):
        ...
        label = obj.label
        id = (
            int(label.split(" ")[-1]) if isinstance(label, str) else int(label)
        )
        box_color = (
            COLOR_PALETTE[id % len(COLOR_PALETTE)] if id >= 0 else (0, 0, 0)
        )
        ...
        if id >= 0:
            label = f"ID {label}" if not isinstance(label, str) else label
    ...
    for track in tracks:
        ...
        if frame_ids[-1] > last_frame_num:
            frame_ids = [
                ...
                if timestamp < last_frame_num
            ]
        xy[track_id][frame_ids] = track_id
        xx = np.where(xy[track_id] == -1, np.nan, x)
        if track_id >= 0:
            color = (
                COLOR_PALETTE[track_id % len(COLOR_PALETTE)]
                if track_id >= 0
                else (0, 0, 0)
            )
