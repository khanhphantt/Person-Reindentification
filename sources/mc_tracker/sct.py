    ...
    def update(self, feature_vec):
        if len(self.clusters) < self.feature_len:
            ...
            idx = random.randint(
                0,
                self.feature_len - 1,
            )  # nosec - disable B311:random check
            self.clusters_sizes[idx] += 1
            self.clusters[idx] += (
                feature_vec - self.clusters[idx]
...
class OrientationFeature:
    def __init__(self, feature_len, initial_feature=(None, None)):
        ...
        self.orientation_features = [
            AverageEstimator() for _ in range(feature_len)
        ]
        self.is_initialized = False
        if (
            initial_feature[0] is not None
            and initial_feature[1] is not None
            and initial_feature[1] >= 0
        ):
            self.is_initialized = True
            self.orientation_features[initial_feature[1]].update(
                initial_feature[0],
            )
    ...
    def dist_to_vec(self, vec, orientation):
        ...
        if (
            orientation >= 0
            and self.orientation_features[orientation].is_valid()
        ):
            return 0.5 * cosine(
                vec,
                self.orientation_features[orientation].get(),
            )
        return 1.0
...
def clusters_distance(clusters1, clusters2):
    if len(clusters1) > 0 and len(clusters2) > 0:
        distances = 0.5 * cdist(
            clusters1.get_clusters_matrix(),
            clusters2.get_clusters_matrix(),
            "cosine",
        )
...
class SingleCameraTracker:
    ...
    def get_tracked_objects(self):
        ...
        for track in self.tracks:
            if (
                track.get_end_time() == self.time - 1
    ...
    def check_and_merge(self, track_source, track_candidate):
        ...
        if (
            idx < 0
        ):  # in this case track already has been modified, merge is invalid
            return

        collisions_found = False
        for i, hist_track in enumerate(self.history_tracks):
            if hist_track.id == id_candidate and not (
                hist_track.get_end_time() < self.tracks[idx].get_start_time()
                or self.tracks[idx].get_end_time()
                < hist_track.get_start_time()
            ):
    ...
    def _continue_tracks(self, detections, features):
        ...
        for i, det1 in enumerate(detections):
            for j, det2 in enumerate(detections):
                if (
                    i != j
                    and self._ios(det1, det2) > self.detection_occlusion_thresh
                ):
        ...
        if cost_matrix.size > 0:
            ...
            for i, j in zip(row_ind, col_ind):
                ...
                if (
                    ...
                    and self._check_velocity_constraint(
                        self.tracks[idx].get_last_box(),
                        self.tracks[idx].get_end_time(),
                        detections[i],
                        self.time,
                    )
                    and self._iou(self.tracks[idx].boxes[-1], detections[i])
                    > self.track_detection_iou_thresh
                ):
                    assignment[i] = j
    ...
    def _clear_old_tracks(self):
        ...
        for track in self.tracks:
            ...
            if (
                track.get_end_time() < self.time - self.continue_time_thresh
                and len(track) < self.time_window
            ):
    ...
    def _rectify_tracks(self):
        ...
        for i, track in enumerate(self.tracks):
            if (
                track.get_end_time() >= self.time - self.rectify_time_thresh
        ...
        distance_matrix = np.zeros(
            (
                len(active_tracks_idx),
                len(not_active_tracks_idx),
            ),
            dtype=np.float32,
        )
    ...
    def _get_merge_distance(self, track1, track2):
        if (
            ...
            if track1.f_orient.is_valid():
                ...
                f_complex_dist = clusters_distance(
                    track1.f_clust,
                    track2.f_clust,
                )
    ...
    def _concatenate_tracks(self, i, idx):
        if self.tracks[i].get_end_time() < self.tracks[idx].get_start_time():
            self.tracks[i].merge_continuation(
                self.tracks[idx],
            ...
            self.tracks[idx].merge_continuation(
                self.tracks[i],
                self.interpolate_time_thresh,
            )
            self.tracks[i] = None
    ...
    def _compute_detections_assignment_cost(
        self,
        active_tracks_idx,
        detections,
        features,
        ...
        cost_matrix = np.zeros(
            (len(detections), len(active_tracks_idx)),
            dtype=np.float32,
        )
        if self.analyzer and len(self.tracks) > 0:
            self.analyzer.prepare_distances(
                self.tracks,
        ...
        for i, idx in enumerate(active_tracks_idx):
            ...
            for j, d in enumerate(detections):
                ...
                if (
                    self.tracks[idx].f_avg.is_valid()
                    and features[j] is not None
                    and self.tracks[idx].get_last_feature() is not None
                ):
                    reid_dist_avg = 0.5 * cosine(
                        self.tracks[idx].f_avg.get(),
                        features[j],
                    )
                    reid_dist_curr = 0.5 * cosine(
                        self.tracks[idx].get_last_feature(),
                        features[j],
                    )
                    ...
                    if self.process_curr_features_number > 0:
                        ...
                        step = -(
                            -num_features // self.process_curr_features_number
                        )
                        step = step if step > 0 else 1
                        start_index = (
                            0
                            if self.process_curr_features_number > 1
                            else num_features - 1
                        )
                        for s in range(start_index, num_features - 1, step):
                            if self.tracks[idx].features[s] is not None:
                                reid_dist_curr = min(
                                    ...
                                    0.5
                                    * cosine(
                                        self.tracks[idx].features[s],
                                        features[j],
                                    ),
                                )

                    reid_dist_clust = clusters_vec_distance(
                        self.tracks[idx].f_clust,
                        features[j],
                    )
                    reid_dist = min(
                        reid_dist_avg,
    ...
    def _check_tracks_velocity_constraint(self, track1, track2):
        if track1.get_end_time() < track2.get_start_time():
            return self._check_velocity_constraint(
                track1.get_last_box(),
                track1.get_end_time(),
                track2.boxes[0],
    ...
    def _check_velocity_constraint(
        self,
        detection1,
        det1_time,
        detection2,
